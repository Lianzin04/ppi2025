...
class Conta:
	#Metodos estaticos(nao pode subcrever) 
    # 
    #e metodos de classe
    ...
    #atributo de classe
	_total_contas = 0
	_lista_contas = [] #atributo de classe(armazena todas as contas criadas)
    #metodo estatico
	@staticmethod
	def total_contas():
	...
	@staticmethod
	def get_saldo_total():
		return reduce(lambda soma, conta: soma + conta.saldo, Conta._lista_contas, 0) #lambda é uma função anonima
	@classmethod
	def total_contas_cm(cls):
		return cls._total_contas

    # metodo de classe (todo self é um metodo de proprio)  
	def __init__(self, cliente, agencia, numero, pix, saldo):
		...
		Conta._total_contas += 1 #metodo estatico
		Conta._lista_contas.append(self) #adiciona a conta criada na lista de contas
